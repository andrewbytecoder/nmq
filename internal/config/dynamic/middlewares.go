...
type CircuitBreaker struct {
	...
	CheckPeriod types.Duration `json:"checkPeriod,omitempty" toml:"checkPeriod,omitempty" yaml:"checkPeriod,omitempty" export:"true"`
	// FallbackDuration is the duration for which the circuit breaker will wait before trying to recover (from a tripped state).
	FallbackDuration types.Duration `json:"fallbackDuration,omitempty" toml:"fallbackDuration,omitempty" yaml:"fallbackDuration,omitempty" export:"true"`
	// RecoveryDuration is the duration for which the circuit breaker will try to recover (as soon as it is in recovering state).
	RecoveryDuration types.Duration `json:"recoveryDuration,omitempty" toml:"recoveryDuration,omitempty" yaml:"recoveryDuration,omitempty" export:"true"`
...
func (c *CircuitBreaker) SetDefaults() {
	c.CheckPeriod = types.Duration(100 * time.Millisecond)
	c.FallbackDuration = types.Duration(10 * time.Second)
	c.RecoveryDuration = types.Duration(10 * time.Second)
...
type Redis struct {
	...
	ReadTimeout *types.Duration `json:"readTimeout,omitempty" toml:"readTimeout,omitempty" yaml:"readTimeout,omitempty" export:"true"`
	// WriteTimeout defines the timeout for socket write operations.
	// Default value is 3 seconds.
	WriteTimeout *types.Duration `json:"writeTimeout,omitempty" toml:"writeTimeout,omitempty" yaml:"writeTimeout,omitempty" export:"true"`
	// DialTimeout sets the timeout for establishing new connections.
	// Default value is 5 seconds.
	DialTimeout *types.Duration `json:"dialTimeout,omitempty" toml:"dialTimeout,omitempty" yaml:"dialTimeout,omitempty" export:"true"`
}
...
func (r *Redis) SetDefaults() {
	...
	defaultReadTimeout := types.Duration(3 * time.Second)
	r.ReadTimeout = &defaultReadTimeout

	defaultWriteTimeout := types.Duration(3 * time.Second)
	r.WriteTimeout = &defaultWriteTimeout

	defaultDialTimeout := types.Duration(5 * time.Second)
