...
func NewCertificateStore(log *zap.Logger, ocspStapler *ocspStapler) *CertificateStore {
...
func (c *CertificateStore) GetBestCertificate(clientHello *tls.ClientHelloInfo) *tls.Certificate {
	...
	if len(serverName) == 0 {
		...
		if err != nil {
			c.log.Error("Could not split host/port", zap.Error(err))
		}
...
func appendCertificate(log *zap.Logger, certs map[string]map[string]*CertificateData, subjectAltNames []string, storeName string, cert *CertificateData) {
	...
	if certExists {
		log.Info("certificate exists", zap.String("certKey", certKey), zap.String("storeName", storeName))
	} else {
		log.Info("add certificate", zap.String("certKey", certKey), zap.String("storeName", storeName))
