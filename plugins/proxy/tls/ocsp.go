...
func (o *ocspStapler) updateStaples(ctx context.Context) {
	for _, item := range o.cache.Items() {
		...
		if err := o.updateStaple(ctx, entry); err != nil {
			o.log.Error("Unable to retieve OCSP staple ", zap.String("CommonName", entry.leaf.Subject.CommonName))
			continue
		}
	}
}
...
func (o *ocspStapler) updateStaple(ctx context.Context, entry *ocspEntry) error {
	...
	for _, responder := range entry.responders {
		o.log.Info("update staple", zap.String("responder", responder))
		...
		if err != nil {
			o.log.Debug("Unable to obtain OCSP response")
			continue
		}
		defer res.Body.Close()

		if res.StatusCode/100 != 2 {
			o.log.Debug("Unable to obtain OCSP response due to status", zap.Int("code", res.StatusCode))
			continue
		}

		ocspResBytes, err := io.ReadAll(res.Body)
		if err != nil {
			o.log.Debug("Unable to read OCSP response bytes")
			continue
		}

		ocspRes, err := ocsp.ParseResponseForCert(ocspResBytes, entry.leaf, entry.issuer)
		if err != nil {
			o.log.Error("Unable to parse OCSP response")
			continue
		}
